...
class CSInfoFrame(LabelFrame):
    ...
    def __init__(self, main_frame: 'CentralPage', x, y, width, height, main_win: Tk, cs: CourseSystem | str, **kw):
        ...
        self.configure(text='Detail Information: ' + self.cs.na)
        self.color_dict: dict[Group | CompoundGroup, str] = {}
        self.color_list = copy.copy(colorList)

        path_label = Label(self, text='Course information file path: ', style='2.TLabel')
        ...
        self.importInfoFile_button = Button(self,
                                            text='Import course information file',
    ...
    def import_info_file(self):
        input_path = filedialog.askopenfilename(title="Choose a file", filetypes=(("csv files", "*.csv"),))
        if input_path != '':
    ...
    def display(self):
        ...
        def switch_two_sessions():
            ...
            if not self.cs.courseTable.switch_two_sns(sni1, sni2):
                messagebox.showwarning('Warning', message='Cannot switch because two sessions are in different modules')
        ...
        def inspect_conflict():
            ...
            if not conflicts:
                message = 'No Conflict...PnP'
            else:
                message = ''
                for conflict in conflicts:
                    message += f'On {conflict[0]}: {"".join([repr(inst) for inst in conflict[1]])}\n'
            messagebox.showinfo('Conflicts', message, parent=root)
            ...
            if not all_conflicts:
                ...
                for conflicts_one_cs, cs in all_conflicts:
                    for conflict in conflicts_one_cs:
                        message += f'On {conflict[0]}:\n    {", ".join([repr(inst) for inst in conflict[1]])}.\n' \
                                   f'    Conflicts with {cs.na}\n'
            messagebox.showinfo('Conflicts', message, parent=root)
        ...
        switch_s_button = Button(upper_frame1, text='Switch two time slots', command=switch_two_slots)
        ...
        switch_c_button = Button(upper_frame1, text='Switch two sessions', command=switch_two_sessions)
        ...
        inspect_button = Button(inspect_frame, text='Inspect', command=inspect_conflict)
        inspect_button.pack(fill='both')
        cross_inspect_button = Button(inspect_frame, text='Cross-Inspect', command=cross_inspect)
        cross_inspect_button.pack(fill='both')
        cross_adjust_button = Button(inspect_frame, text='Cross-Adjust', command=cross_adjust)
        ...
        save_button = Button(root, text='Export .csv', command=export_csv)
...
class CentralPage(Frame):
    def __init__(self, master: Tk, **kw):
        ...
        self.tableFormatFrame = LabelFrame(self, text='Table Format')
        ...
        self.createCs_b = Button(self.courseSystemOperateFrame, text='Add...', command=self.create_cs)
        self.createCs_b.grid(column=0, row=1, sticky='w')

        self.importCs_b = Button(self.courseSystemOperateFrame, text='Import from draft', command=self.import_cs)
        ...
        name = simpledialog.askstring(title='Welcome',
                                      prompt='Welcome to use ACAS2.0. Please indicate the name of your school')
        if not name:
    ...
    def create_cs(self):
        new_cs_name = simpledialog.askstring(title='New Course System',
                                             prompt='Please enter the name of this course system')
        if new_cs_name:
            # check if exist...
    ...
    def import_cs(self):
        open_path = filedialog.askopenfilename(title="Choose a file", filetypes=(("txt files", "*.txt"),))
        if open_path:
